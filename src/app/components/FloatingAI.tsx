...
function getQuickReply(q: string): string {
  ...
  if (t.includes('honeymoon') || t.includes('romantic')) return "For a romantic stay I'd recommend our Sunset Overwater Cottage — glass floor over the lagoon, private plunge pool 🌅. Want me to check availability?";
  if (t.includes('div')) return "Our PADI dive center runs 3 sessions daily from 6 AM. Open Water starts at ₱2,800/person 🤿. Certification required for Night Diving.";
  if (t.includes('snorkel')) return "Snorkeling Safari runs 4× daily and is perfect for all ages (kids 5+) at ₱1,200/person 🐠. Gear included!";
  if (t.includes('island hop')) return "Island Hopping departs at 7:30 AM daily — 3 islands, lunch, and snorkeling gear all included for ₱2,200 🏝️.";
  if (t.includes('family') || t.includes('kids')) return "Our Pearl Family Bungalow sleeps 5 with two bedrooms and a private garden. Best value for families 👨‍👩‍👧‍👦.";
  if (t.includes('price') || t.includes('cost') || t.includes('cheap')) return "Rooms range from ₱3,200/night (Garden Cottage) to ₱11,500/night (Overwater Cottage). Activities from ₱800. Shall I find something in your budget?";
  if (t.includes('cancel')) return "Free cancellation up to 48 hours before check-in. Within 48 hours, one night's charge applies.";
  if (t.includes('check') && (t.includes('in') || t.includes('out'))) return "Check-in from 2:00 PM · Check-out by 12:00 PM noon. Early/late arrangements available on request!";
  if (t.includes('pay') || t.includes('gcash') || t.includes('card')) return "We accept credit/debit cards, GCash, and bank transfer — all secured with 256-bit SSL encryption 🔒.";
  if (t.includes('hello') || t.includes('hi') || t.includes('hey')) return "Hello there! 👋 I'm Maya, your Sabang Beach and Diving Resort AI concierge. Ask me about rooms, diving, island hopping, or anything about the resort!";
...
export default function FloatingAI() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { role: 'ai', text: `Hi${user ? ` ${user.firstName}` : ''}! 👋 I'm Maya — ask me anything about Sabang Beach and Diving Resort.` },
  ]);
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        {nudge && !open && (
          <div className="bg-white border border-border rounded-2xl shadow-xl px-4 py-3 max-w-[220px] animate-in slide-in-from-bottom-2 fade-in duration-300">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">Ask Maya, your AI concierge!</p>
        ...
        <button
          ...
          style={{ boxShadow: open ? undefined : '0 8px 32px rgba(10,61,98,0.35)' }}
        >
          {open ? (
      ...
      {open && (
        <div
          ...
          style={{ maxHeight: '520px', boxShadow: '0 24px 64px rgba(10,61,98,0.18)' }}
        >
          {/* Header */}
            ...
            {typing && (
              <div className="flex gap-2">
                ...
                <div className="bg-muted px-3 py-2 rounded-2xl rounded-tl-sm flex items-center gap-1">
                  {[0, 0.15, 0.3].map((d, i) => (
                    <span key={i} className="w-1.5 h-1.5 bg-muted-foreground/50 rounded-full animate-bounce" style={{ animationDelay: `${d}s` }} />
                  ))}
                </div>
              </div>
            )}
